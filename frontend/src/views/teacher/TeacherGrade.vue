...
<script setup>
// 教师-批改工作台：整个 AI 批改流程的操作页。
// 选评分标准和模型 → 生成 AI 建议（分数/评语/错误框）→ 教师在画布上
// 调整错误框和扣分、修改分数评语 → 确认后结果对学生可见；也可重置重来。
...
// 状态以 review_status 为主；score 非空是兼容早期已评分记录的兜底判断。
...
// 用作业数据填充表单：优先取 AI 草稿字段，没有草稿时取已确认结果
function syncReviewForm(data) {
  // 重新生成建议会清空最终结果；这里的优先级保证“待复核”界面始终展示最新 AI 草稿。
...
// 并发加载页面所需的三份数据：作业详情、评分标准列表、可用模型列表
async function loadHomework() {
  const [{ data: homework }, { data: criteriaData }, { data: modelData }] = await Promise.all([
    teacherApi.fetchHomework(route.params.id),
    teacherApi.fetchCriteria(),
    teacherApi.fetchAiModels(),
  ])
  hw.value = homework
  criteria.value = criteriaData
  aiModels.value = modelData
  // 优先回显上一次 AI 批改选择的标准；若它已删除/不可用，再选当前第一条可用标准。
...
async function generateAiReview() {
  ...
  try {
    // 后端会原子地覆盖旧草稿和最终结果，返回的新作业记录就是页面唯一数据源。
    const { data } = await teacherApi.generateAiReview(route.params.id, {
...
// 完成复核：前端先做一轮校验（分数范围、评语非空、扣分合法）再提交
async function confirmReview() {
  ...
  try {
    // errorBoxes 已由画布换回 source_pixel；后端会再次校验坐标范围，不能只依赖前端。
    const { data } = await teacherApi.submitGrade(route.params.id, { score: reviewForm.score, comment: reviewForm.comment, error_boxes: errorBoxes.value })
    hw.value = data
    syncReviewForm(data)
  } catch (err) {
...
// 重置批改：清空 AI 草稿和已确认结果，回到待批改状态
async function resetReview() {
  requestError.value = ''
  resetting.value = true
  try {
    const { data } = await teacherApi.resetAiReview(route.params.id)
    hw.value = data
    syncReviewForm(data)
  } catch (err) {
