...
<script setup>
// 回收站（学生/教师共用）：路由 meta.role 决定当前身份，
// 据此选用对应的接口模块；恢复和彻底删除都只影响本角色侧
...
// 按角色选择接口模块（两个模块的回收站函数同名同签名）
const roleApi = computed(() => (role.value === 'teacher' ? teacherApi : studentApi))
...
async function loadRecycleBin() {
  const { data } = await roleApi.value.fetchRecycleBin()
...
function restore(id) {
  return runAction(id, () => roleApi.value.restoreHomework(id))
}

function permanentlyDelete(id) {
  return runAction(id, () => roleApi.value.permanentlyDeleteHomework(id))
}
